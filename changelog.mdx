...
<AccordionGroup>
  <Accordion title="New: visitor_id (beta)">
    Added `visitor_id` to the session, user, and tag API endpoints and to the session details modal on the dashboard. The visitor ID is a fingerprint of the user's device — sessions with the same visitor ID likely came from the same device, even if the user ID differs. Currently in beta.
  </Accordion>
